...
export class DatabaseService {
    ...
    async getProducts() {
        const client = await this.pool.connect();
        try {
            const result = await client.query('SELECT * FROM products WHERE is_active = true ORDER BY product_type');
            return result.rows;
        }
        finally {
            client.release();
    ...
    async getProduct(productType) {
        const client = await this.pool.connect();
        try {
            const result = await client.query('SELECT * FROM products WHERE product_type = $1', [productType]);
    ...
    async saveQuote(quote) {
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO quotes (quote_id, product_type, applicant_data, pricing_result, eligibility_flags, created_at, expires_at)
    ...
    async deleteExpiredQuotes() {
        const client = await this.pool.connect();
        try {
            const result = await client.query('DELETE FROM quotes WHERE expires_at < NOW()');
    ...
    async getRateTable(productType, riskClass, gender, age, termLength) {
        const client = await this.pool.connect();
        try {
            let query = `
